...
/* Declaration of buzzer_driver.c functions */

static int buzzer_driver_init(void);
static void buzzer_driver_exit(void);
static int buzzer_driver_open(struct inode *, struct file *);
static int buzzer_driver_release(struct inode *, struct file *);
static ssize_t buzzer_driver_read(struct file *, char *buf, size_t , loff_t *);
static ssize_t buzzer_driver_write(struct file *, const char *buf, size_t , loff_t *);
...
{
    .owner = THIS_MODULE,
    .open = buzzer_driver_open,
    .write = buzzer_driver_write,
    .read = buzzer_driver_read,
};
...
static int __init buzzer_driver_init()
{
    ...
    printk(KERN_INFO "Inserting buzzer_driver module\n");
    ...
    /* Registering device. */
    result = register_chrdev(0, "buzzer_driver", &gpio_driver_fops);
    if (result < 0)
    {
        printk(KERN_INFO "buzzer_driver: cannot obtain major number %d\n", gpio_driver_major);
        return result;
    }

    gpio_driver_major = result;
    printk(KERN_INFO "buzzer_driver major number is %d\n", gpio_driver_major);
...
fail_no_mem:
    ...
    unregister_chrdev(gpio_driver_major, "buzzer_driver");
...
static void __exit buzzer_driver_exit(void) 
{
    ...
    printk(KERN_INFO "Buzzer driver exited\n");
}

/* File open function. */
static int buzzer_driver_open(struct inode *inode, struct file *file) 
{
    return 0;
}

/* File close function. */
static int buzzer_driver_release(struct inode *inode, struct file *filp)
{
    /* Success. */
    return 0;
}
 ...
 */
static ssize_t buzzer_driver_read(struct file *file, char __user *buffer, size_t len, loff_t *offset) 
{
    /* You can't read from buzzer. */
    return 0; 
}
