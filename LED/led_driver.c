 ...
 * The bus addresses for peripherals are set up to map onto the peripheral bus address range
...
typedef enum {
	PULL_NONE = 0, 		// 00 = Off – disable pull-up/down
	PULL_DOWN = 1, 	// 01 = Enable Pull Down control 
	PULL_UP = 2			// 10 = Enable Pull Up control 
} PUD;
 ...
 */
uint32_t GetGPFSELReg(char pin)
{
    uint32_t addr;
...
int gpio_driver_init(void)
{
    ...
    /* LEDS */
    SetGpioPinDirection(GPIO_06, GPIO_DIRECTION_OUT); //LED0[YELLOW DIODE]
...
fail_irq:
    printk(KERN_INFO "fail_irq has been called!\n");
